...
async function Details(type, id, movie='') {
  ...
  stop1.setAttribute('stop-color', '#5ee91e');
  stop2.setAttribute('offset', '110%');
  stop2.setAttribute('stop-color', '#cee91e');
  ...
  if (type !== 'person') {
    ...
    MovieCard(`${API}/${type}/${id}/similar`, topMovies)
  ...
  details.append(response.overview ?? response.biography)
...
async function MovieCard(url, wrapper, ) {
  ...
  response.results.forEach(element => {
    ...
    const date = document.createElement('article')
    // classes
    card.classList.add('me-2', 'mt-3')
    movideImg.classList.add('portada');
